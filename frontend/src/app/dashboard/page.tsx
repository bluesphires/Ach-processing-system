...
import { TransactionStats, NACHAGenerationStats } from '@/types';
...
export default function DashboardPage() {
  ...
  const [nachaStats, setNACHAStats] = useState<NACHAGenerationStats | null>(null);
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [transactionResponse, nachaResponse] = await Promise.all([
        apiClient.getTransactionStats(),
        apiClient.getNACHAGenerationStats()
      ]);
      ...
      if (nachaResponse.success && nachaResponse.data) {
        setNACHAStats(nachaResponse.data);
      }
    } catch (error) {
  ...
  const stats: StatCard[] = [
    ...
    },
    {
      ...
      value: nachaStats ? formatNumber(nachaStats.totalFiles) : '0',
